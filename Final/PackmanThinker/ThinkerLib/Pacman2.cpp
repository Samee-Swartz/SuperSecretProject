...
Direction::Enum OnPacmanThink2(const Pawn& in_ourPawn, const World& in_ourWorld,
...
// depth first in all four directions
	...
	{ // loop through directions
		con = curNode->GetConnection(static_cast<Direction::Enum>(i)); // get connection in that direction
		if (con.IsValid())
		{ // is it valid?
...
// run depthfirst on this new node. it will return a score for that node. add it to counts with the direction the node was in
			counts.push_back(std::make_pair(DepthFirst(in_ourWorld,
			                                           in_ourWorld.GetNode(con.GetOtherNodeId()), nodes) - s, static_cast<Direction::Enum>(i)));
		}
	}
...
// unit vector from pacman to closest node
	Vector2 fromPacman = Vector2(curNode->GetPosition() - in_ourPawn.GetPosition()).GetNormalizedVector();
