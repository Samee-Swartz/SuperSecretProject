...
Direction OnInkyThink(const Pawn& in_ourPawn, const World& in_ourWorld, float in_deltaTime, float in_totalTime){
...
//run A* from the closest node to Inky to the closest node that Pacman is occupying
Direction aStar(currentNode, targetNode){
	std::Queue<ScoredNode> bestFirstQueue;
	std::Queue<ScoredNode> tempQueue;
	
	Expand(in_ourWorld, currentNode, bestFirstQueue);
	
	while(bestFirstQueue.front() != targetNode){
